...
- **중복 제목 구분** — 자동완성·검색·탭·그래프(PiP 포함)에 구분 가능한 최소 폴더 경로를 표시하고, 마우스 오버 시 전체 상대 경로 제공
- **위키 링크 자동완성** — `[[` 입력 후 제목·경로 검색, ↑/↓ 선택, Enter·Tab·클릭 삽입, Esc 닫기. 같은 제목은 폴더 경로로 구분
- **백링크 패널** — 사이드바 백링크 버튼으로 현재 노트를 참조하는 노트와 링크 문맥을 확인하고 원본 노트로 이동. 열린 노트의 저장 전 변경도 반영하며 외부 파일 변경은 패널 새로고침으로 반영
...
- **드래그 도킹** — 그래프 제목 영역을 메인 편집 영역 좌우 가장자리에 놓아 배치
- **별도 창(PiP)** — Electron에서 그래프 제목을 앱 창 밖으로 드래그하거나 기존 새 창 버튼으로 분리
- **재결합** — PiP 그래프 제목을 메인 편집 영역 가장자리로 드래그하거나 PiP의 좌우 도킹 버튼 사용
- **배치 복원** — 그래프 위치·너비·접힘/분리 상태 저장, 창 분리·재결합 시 노드 좌표 유지
...
`npm run test:graph-docking`으로 빌드 후 숨겨진 Electron 창에서 도킹, 크기 조절, 창 분리·재결합, 노드 좌표와 배치 복원을 검증합니다. 사용자 노트와 분리된 테스트 폴더 및 저장소를 사용합니다.
드래그 이벤트와 외부 커서 위치는 테스트에서 주입하므로 실제 마우스의 창 밖 드래그와 다중 모니터 동작은 수동 확인이 필요합니다.

위키 링크 자동완성은 `npm run test:wikilinks`로 검증합니다. 일치하는 노트가 없으면 ‘새 노트 만들기’를 선택해 현재 노트와 같은 폴더에 생성하고 링크를 삽입할 수 있습니다. 폴더 경로 입력과 단독으로 연 파일에서는 생성을 제공하지 않습니다. 코드 블록과 인라인 코드에서는 자동완성을 띄우지 않습니다.
